...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-carbon-900 relative overflow-hidden px-4">
      <div className="absolute -top-32 -left-32 w-96 h-96 bg-papaya-500 rounded-full blur-3xl opacity-20" />
      <div className="absolute -bottom-32 -right-32 w-96 h-96 bg-racingblue-500 rounded-full blur-3xl opacity-20" />
      ...
      >
        <div>
          <h1 className="text-2xl font-extrabold text-carbon-900">Vamos começar! 🚀</h1>
          <p className="text-sm text-carbon-500 mt-1">
            Crie sua conta e comece a revisar seu código com mais confiança.
          </p>
        </div>
        {error && <p className="text-sm text-red-600">{error}</p>}
        <div>
          <label className="block text-sm text-carbon-600 mb-1">Nome</label>
          <input
            required
            value={name}
        ...
        <div>
          <label className="block text-sm text-carbon-600 mb-1">Senha (mín. 8 caracteres)</label>
          <input
            type="password"
            required
            minLength={8}
        ...
        <p className="text-sm text-carbon-500 text-center">
          Já tem conta?{" "}
          <Link to="/login" className="text-papaya-600 font-medium hover:underline">
            Entrar
          </Link>
        </p>
      </form>
